...
@Repository
public interface PrimaryContactRepository extends JpaRepository<PrimaryContact, UUID> {
...
//	List<Permission>findAllPermissionsForRestaurant(@Param("roleId")UUID roleId,@Param("restaurantPermissionId")UUID restaurantPermissionId);
    ...
    @Query("select u from PrimaryContact u where u.id = :id")
    PrimaryContact findByUUID(@Param("id") UUID id);
